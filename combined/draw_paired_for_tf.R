...
file.fig <- "figures/paired_for_tfs.pdf"
file.mat <- "derived_data/mat.transformed.pval.tf.rds"

list.link <- vector("list", 5)
# list.link[[1]] <- "derived_data/linked.TF.aggr.csv" %>%
#     read.csv()
list.link[[1]] <- "derived_data/linked.TF.csv" %>%
    read.csv()
list.link[[2]] <- "derived_data/linked.TF.DMSO.csv" %>%
    read.csv()
list.link[[3]] <- "derived_data/linked.TF.iBET151.csv" %>%
    read.csv()
list.link[[4]] <- "derived_data/linked.TF.Prostrat.csv" %>%
    read.csv()
list.link[[5]] <- "derived_data/linked.TF.SAHA.csv" %>%
    read.csv()

# take p-values from simple linear regression

lapply(list.link, head)
tfs <- list.link %>%
...
sel <- c("pval", "pval.pea")
tmp <- list.link %>%
    lapply(function(x) x[match(tfs, x$TF), ]) %>%
